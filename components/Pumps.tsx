...
import FeaturedPumpCard from './FeaturedPumpCard';
import PumpCard from './PumpCard';
import PumpDetailModal from './PumpDetailModal';

const Pumps = () => {
  const [selectedPump, setSelectedPump] = useState<PumpDetail | null>(null);

  const handleKnowMore = (pumpId: string) => {
    const pump = pumpDetails.find(p => p.id === pumpId);
        ...
        <div className="max-w-[1400px] mx-auto px-6 py-20">
          ...
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
              <FeaturedPumpCard
                title="High Water Flow Technology Improves Pump Efficiency"
                imageUrl="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSvC67-aItDxlylhda4j4MKfkBAgGKAxjJXVA&s"
                features={['High water flow technology', 'Improved pump efficiency']}
                gradientFrom="from-blue-50"
                gradientTo="to-blue-100"
                onKnowMore={() => handleKnowMore('high-water-flow')}
              />

              <FeaturedPumpCard
                title="5 Benefits of Using Residential Pumps in Your Home"
                imageUrl="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTha1ZjY9PV6oImAIvHeper7gqqJ9Ug1gv5Eg&s"
                features={['Easy to use operation', 'User-friendly design']}
                gradientFrom="from-green-50"
                gradientTo="to-green-100"
                onKnowMore={() => handleKnowMore('residential-pumps')}
              />

              <FeaturedPumpCard
                title="Difference Between Centrifugal Pump and Reciprocating Pump Explained"
                imageUrl="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSLVSxMWkqaoFpXh1MD03TAOGGzBXFiUTz3kg&s"
                features={['Easy to use operation', 'User-friendly design']}
                gradientFrom="from-purple-50"
                gradientTo="to-purple-100"
                onKnowMore={() => handleKnowMore('centrifugal-reciprocating')}
              />

              <FeaturedPumpCard
                title="Top 3 Agricultural Pumps for Better Farm Productivity"
                imageUrl="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQhOnZpZ9s_S6q-6FAs0M-hZJ5Vmz_8LhKmdQ&s"
                features={['Agricultural-specific features', 'Enhanced farm productivity']}
                gradientFrom="from-amber-50"
                gradientTo="to-amber-100"
                onKnowMore={() => handleKnowMore('agricultural-pumps')}
              />
            </div>
          </div>
